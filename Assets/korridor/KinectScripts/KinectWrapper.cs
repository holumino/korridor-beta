...
public class KinectWrapper : MonoBehaviour
{
	...
	public static class Constants
	{
		...
		public static float PoseCompleteDuration = 1.0f;
	}
...
//						else 
			...
			case Gestures.ZoomOut:
				...
				{
					case 0:  // gesture detection - phase 1
						Vector3 distZoomOut = jointsPos[rightHandIndex] - jointsPos[leftHandIndex];
				
						if(jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] &&
						   jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
					       (jointsPos[leftHandIndex].y - jointsPos[leftElbowIndex].y) > 0f &&
					       (jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) > 0f &&
						   distZoomOut.sqrMagnitude < 0.1f)
						{
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
							gestureData.progress = 0.3f;
						}
			...
			case Gestures.ZoomIn:
				...
				{
					case 0:  // gesture detection - phase 1
						Vector3 distZoomIn = jointsPos[rightHandIndex] - jointsPos[leftHandIndex];

						if(jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] &&
						   jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
					       (jointsPos[leftHandIndex].y - jointsPos[leftElbowIndex].y) > 0f &&
					       (jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) > 0f &&
						   distZoomIn.sqrMagnitude >= 0.5f)
						{
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
							gestureData.tagFloat = distZoomIn.magnitude;
							gestureData.progress = 0.3f;
						}
						break;
